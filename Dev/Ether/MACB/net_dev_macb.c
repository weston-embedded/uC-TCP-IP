...
#define    NET_DEV_MACB_MODULE

#include  <Source/net.h>
#include  <Source/net_util.h>
#include  <IF/net_if.h>
#include  <IF/net_if_ether.h>
#include  "net_dev_macb.h"
...
typedef  struct  dev_desc {                                     /* See Note #2.                                         */
    CPU_REG32    Addr;                                          /* Buffer address.                                      */
    CPU_REG32    Status;                                        /* Buffer status.                                       */
} DEV_DESC;
...
static  void  NetDev_IntEn              (NET_IF             *pif);

static  void  NetDev_IntDis             (NET_IF             *pif);

static  void  NetDev_RxIntDis            (NET_IF            *pif);

static  void  NetDev_RxIntEn            (NET_IF             *pif);
...
*********************************************************************************************************
*/
                                                                                    /* MACB dev API fnct ptrs :         */
const  NET_DEV_API_ETHER  NetDev_API_MACB = { NetDev_Init,                          /*   Init/add                       */
                                              NetDev_Start,                         /*   Start                          */
                                              NetDev_Stop,                          /*   Stop                           */
                                              NetDev_Rx,                            /*   Rx                             */
                                              NetDev_Tx,                            /*   Tx                             */
...
#define  EMAC_TXBUF_STATUS_BUF_NONE               DEF_BIT_27    /* Buffers exhausted in mid frame                       */
#define  EMAC_TXBUF_STATUS_UNDR                   DEF_BIT_28    /* Transmit underrun                                    */
#define  EMAC_TXBUF_STATUS_RETRY                  DEF_BIT_29    /* Retry Limit exceeded                                 */
...
*                   (e) pdev_cfg->RxDescNbr
*
*                       Configuration of the MACB etnerhet controller on the RX side is a special
*                       case since it restrict the buffer size to 128 bytes. The driver allocates
*                       chunks of 128 bytes so the number of descriptor has to be much higher
*                       than the number of buffer. The 50% heuristic rule should take in
*                       consideration the MAC buffer size restriction of 128 bytes:
*
*                           RxDescNbr > (RxBufLargeNbr * RxBufLargeSize * 50%)
*                                        -----------------------------------
*                                                     128
*
*                   (f) pdev_cfg->TxDescNbr
*
                           ...
                           NET_ERR  *perr)
{
    ...
    if ((rx_buf_size % RX_DESC_BUF_ALIGN) != 0) {
        *perr = NET_DEV_ERR_INVALID_CFG;                        /* Descriptors AND Rx buffers MUST be aligned on a ...  */
         return;                                                /* ... 32 BYTE boundary.                                */
                                                                ...
                                                                /* Validate Rx desc nbr. See Note #7e.                  */
    if (pdev_cfg->RxDescNbr < ((pdev_cfg->RxBufLargeNbr * pdev_cfg->RxBufLargeSize * 50)/ (128 * 100))) {
    ...
    NetDev_IntDis(pif);                                         /* Disable all int. src's.                              */
...
#ifdef  NET_MCAST_MODULE_EN
    pdev->MACB_NCFGR |= MACB_NCFGR_MTI;                         /* Enable multicast hash.                               */
#endif

    pdev->MACB_USRIO  = MACB_USRIO_CLKEN;                       /* Enable tranceiver input clk (EAM on AVR32 MACB).     */
                            ...
                            NET_ERR  *perr)
{
    ...
    if (*perr != NET_DEV_ERR_NONE) {
         return;
                                                                ...
                                                                /* -------------------- CFG INT'S --------------------- */
    pdev->MACB_RSR = (MACB_RSR_OVR  |                           /* Clear rx status reg bits.                            */
                      MACB_RSR_REC  |
                      MACB_RSR_BNA);

    pdev->MACB_TSR = (MACB_TSR_UBR  |                           /* Clear tx reg bits.                                   */
                      MACB_TSR_COL  |
                      MACB_TSR_RLES |
                      MACB_TSR_BEX  |
                      MACB_TSR_COMP |
    ...
    NetDev_IntEn(pif);                                          /* Enable dev int. src's.                               */
                           ...
                           NET_ERR  *perr)
{
    ...
    for (i = 0; i < pdev_cfg->TxDescNbr; i++) {
        if ((pdesc->Status & EMAC_TXBUF_STATUS_USED) == 0) {    /* If NOT yet  tx'd, ...                                */
                         ...
                         NET_ERR      *perr)
{
    ...
    CPU_BOOLEAN         frame_found;
    CPU_BOOLEAN         frame_buf_avail;
    ...
    if ((src_addr & EMAC_RXBUF_SW_OWNED) == DEF_NO) {           /* Error.  Descriptor not yet owned by software.        */
        ...
        *perr   = (NET_ERR     )NET_DEV_ERR_RX;
         NetDev_RxIntEn(pif);                                   /* Enable Rx interupt                                   */
         return;
    }
    ...
    if ((status & EMAC_RXBUF_SOF_MASK) == DEF_NO) {
        ...
        *p_data = (CPU_INT08U *)0;
        *perr   = (NET_ERR     )NET_DEV_ERR_RX;
         NetDev_RxIntEn(pif);                                   /* Enable Rx interrupt                                 */
         return;
    }
    ...
    while (frame_found == DEF_NO) {
        if (DEF_BIT_IS_SET(status, EMAC_RXBUF_EOF_MASK)) {      /* Check for end of frame                               */
            src_addr  = pdesc->Addr & EMAC_RXBUF_ADDRESS_MASK;
            frame_len = status      & EMAC_RXBUF_LEN_MASK;
            copy_len  = frame_len - dst_offset;

            if ((frame_len >= NET_IF_ETHER_FRAME_MIN_SIZE ) &&
                (frame_len <= pdev_cfg->RxBufLargeSize    )) {

                if (frame_buf_avail == DEF_YES) {
            ...
            if (pdev_data->RxNRdyCtr > 0u) {
                pdev_data->RxNRdyCtr--;                         /* Update number of frames to process                   */
            }
        ...
        if (DEF_BIT_IS_CLR(src_addr, EMAC_RXBUF_SW_OWNED)) {    /* Validate ownership (see Note #4).                    */
            NET_CTR_ERR_INC(pdev_data->ErrRxPktOwner2);
            *perr = NET_DEV_ERR_RX;
            break;
        }

        if (DEF_BIT_IS_SET(status, EMAC_RXBUF_SOF_MASK)) {      /* Premature SOF before EOF.  Incomplete frame found.   */
...
*                                           NetDev_RxIntEn()
*
* Description : Enable Rx interrupt sources.
*
...
static void NetDev_RxIntEn (NET_IF *pif)
{
    ...
    NET_DEV           *pdev;
    NET_ERR            err;
                                                                /* ------- OBTEIN REFERENCE TO DEVICE REGISTERS ------- */
    pdev_cfg  = (NET_DEV_CFG_ETHER *)pif->Dev_Cfg;              /* Obtain ptr to the dev cfg struct.                    */
    pdev_data = (NET_DEV_DATA      *)pif->Dev_Data;
    pdev      = (NET_DEV           *)pdev_cfg->BaseAddr;        /* Overlay dev reg struct on top of the dev base addr.  */

    pdev_data->RxNRdyCtr += pdev->MACB_FRO;                     /* Update number of frames to process                   */
...
*                                           NetDev_RxIntDis()
*
* Description : Disable Rx interrupt sources.
*
* Argument(s) : pif     Pointer to interface requiring service.
*
* Return(s)   : none.
...
static  void  NetDev_RxIntDis (NET_IF  *pif)
{
    ...
    pdev     = (NET_DEV           *)pdev_cfg->BaseAddr;         /* Overlay dev reg struct on top of dev base addr.      */
                                                                ...
                                                                /* --------------- DISABLE INT SRC'S ------------------ */
    pdev->MACB_IDR = (MACB_ISR_RCOMP |                          /* Disable 'Reception complete' interrupt.              */
                      MACB_ISR_ROVR);                           /* Disable 'Receiver overrun'   interrupt.              */
                         ...
                         NET_ERR     *perr)
{
    ...
    if ((desc_status & EMAC_TXBUF_STATUS_USED) > 0) {           /* Check if descriptor is software owned (see Note #3). */
        ...
        pdesc->Addr     =  (CPU_INT32U)p_data;                  /* Configure descriptor with Tx data area address.      */

        desc_status    &= ~(EMAC_TXBUF_STATUS_USED     |        /* Mark desc as NOT having been consumed by DMA.        */
                            EMAC_TXBUF_STATUS_UNDR     |        /* Mark desc as NOT having a Transmit underrun           */
                            EMAC_TXBUF_STATUS_BUF_NONE |        /* Mark desc as NOT being exhausted in mid frame        */
                            EMAC_TXBUF_TX_SIZE_MASK);           /* Clr frame len bits.                                  */

        desc_status    |=   size & EMAC_TXBUF_TX_SIZE_MASK;     /* Cfg Tx len.                                          */
        pdesc->Status   =   desc_status;                        /* Update desc.                                         */

        pdev->MACB_NCR |=   MACB_NCR_TSTART;                    /* Start Tx.                                            */
                                 ...
                                 NET_ERR  *perr)
{
    ...
    pdev->MACB_RBQP = (CPU_INT32U)pdev_data->RxBufDescPtrStart;
    reg_val         = pdev->MACB_FRO;                           /* Assignment correct?                                  */
    (void)&reg_val;
                                 ...
                                 NET_ERR *perr)
{
    ...
    pdev->MACB_TBQP = (CPU_INT32U)pdev_data->TxBufDescPtrStart;
                                  ...
                                  NET_DEV_ISR_TYPE   type)
{
   ...
   (void)&isr_stat;
    ...
    pdev      = (NET_DEV           *)pdev_cfg->BaseAddr;        /* Overlay dev reg struct on top of dev base addr.      */
                                                                ...
                                                                /* ------------------- PROCESS ISR -------------------- */
    isr_stat  = pdev->MACB_ISR;                                 /* Clear int. status reg.                               */
    isr_mask  = pdev->MACB_IMR;


                                                                /* ----------------- HANDLE RX EVENT ------------------ */
    if (((~isr_mask & (MACB_ISR_RCOMP | MACB_ISR_ROVR)) > 0)) {
        reg_val  = pdev->MACB_RSR;                              /* Read the Receive Status Register                     */
        rx_event = DEF_NO;
        if (((~isr_mask & MACB_ISR_RCOMP) > 0) &&
            ((reg_val   & MACB_RSR_REC  ) == MACB_RSR_REC)) {   /* Check if we have recieved a frame                    */
             pdev->MACB_RSR = MACB_RSR_REC;                     /* Clear the Receive Status Register REC bit            */
             rx_event = DEF_YES;
        }

        if (((~isr_mask & MACB_ISR_ROVR) > 0) &&
            ((reg_val  & (MACB_RSR_OVR)) > 0)) {                /* Clear receiver overrun errors if they have occured  */
            pdev->MACB_RSR = reg_val & (MACB_RSR_OVR);
            rx_event = DEF_YES;
        }
        ...
        if (rx_event == DEF_YES) {
            ...
            switch (err) {
                case NET_IF_ERR_NONE:
                     NetDev_RxIntDis(pif);                      /* Disable Rx interrupts                                */
                     break;
    ...
    if (reg_val & (MACB_TSR_COMP |                              /* Check if frame successfully completed Tx, or if  ... */
                   MACB_TSR_UND  |                              /* ... passive  error occured,                      ... */
                   MACB_TSR_BEX  |                              /* ... buffer exhausted mid-frame,                  ... */
                   MACB_TSR_RLES)) {

        if (reg_val & (MACB_TSR_COMP |                          /* Clr int. src's.                                      */
                       MACB_TSR_BEX  |
                       MACB_TSR_RLES)) {

            pdev->MACB_TSR = reg_val & (MACB_TSR_COMP |
                                        MACB_TSR_BEX  |
                                        MACB_TSR_RLES |
                                        MACB_TSR_COL);
        }
        ...
        if ((pdesc->Status & EMAC_TXBUF_STATUS_USED) > 0) {
                                                                /* Check for transmit error on retry limit exceeded    */
            if (DEF_BIT_IS_SET(pdesc->Status, EMAC_TXBUF_STATUS_RETRY)) {
                tx_col = DEF_TRUE;
                for (i = 0; i < pdev_cfg->TxDescNbr; i++) {
                    pdesc = &pdev_data->TxBufDescPtrStart[i];
                    if (((pdesc->Status & EMAC_TXBUF_STATUS_USED ) == 0) ||
                        ((pdesc->Status & EMAC_TXBUF_TX_SIZE_MASK) != 0)) {
                        NetIF_TxDeallocTaskPost((CPU_INT08U *)pdesc->Addr, &err);
                    }
                }
                 ...
                 NetIF_DevCfgTxRdySignal(             pif,      /* Re-cfg Tx Rdy sem count.                             */
            ...
            if (!tx_col) {
                ...
                if (pdesc != pdev_data->TxBufDescPtrEnd) {      /* Point to next descriptor.                            */
                    pdesc++;
                } else {
...
*                                            NetDev_IntEn()
*
* Description : Enable Rx and Tx interrupt sources.
*
* Argument(s) : pif     Pointer to interface requiring service.
*
*               perr    Pointer to return error code.
...
static  void  NetDev_IntEn (NET_IF  *pif)
{
    ...
    pdev->MACB_IER = (MACB_ISR_RCOMP |                          /* Enable 'Reception complete' interrupt.               */
                      ...
                      MACB_ISR_RLEX  |                          /* Enable Tx retry limit exceeded interrupt.            */
                      MACB_ISR_TXERR |                          /* Enable Tx buffer exhasted mid frame interrupt.       */
                      MACB_ISR_HRESP);                          /* Enable HRESP not OK interrupt.                       */
}
...
*                                           NetDev_IntDis()
*
* Description : Disable Rx and Tx interrupt sources.
*
* Argument(s) : pif     Pointer to interface requiring service.
*
*               perr    Pointer to return error code.
...
static  void  NetDev_IntDis (NET_IF  *pif)
{
    ...
    pdev->MACB_IDR = (MACB_ISR_RCOMP |                          /* Disable 'Reception complete' interrupt.              */
                      MACB_ISR_ROVR  |                          /* Disable 'Receiver overrun'   interrupt.              */
                      MACB_ISR_TCOMP |                          /* Disable Tx complete interrupt                        */
                      MACB_ISR_TUNDR |                          /* Disable Tx underrun interrupt.                       */
                      MACB_ISR_RLEX  |                          /* Disable Tx retry limit exceeded interrupt.           */
                      MACB_ISR_TXERR |                          /* Disable Tx buffer exhasted mid frame interrupt.      */
                      MACB_ISR_HRESP);                          /* Disable HRESP not OK interrupt.                      */
}
