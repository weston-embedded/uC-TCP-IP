...
#define    NET_DEV_SH_ETHERC_MODULE
#include  <Source/net.h>
#include  <IP/IPv4/net_igmp.h>
#include  <IF/net_if.h>
#include  <IF/net_if_ether.h>
#include  <lib_mem.h>
#include  "net_dev_sh_etherc.h"
...
*********************************************************************************************************
*/
                                                                                    /* SH-Ether dev API fnct ptrs :     */
const  NET_DEV_API_ETHER  NetDev_API_SH_EtherC = { NetDev_Init,                     /*   Init/add                       */
                                                   NetDev_Start,                    /*   Start                          */
                                                   NetDev_Stop,                     /*   Stop                           */
                                                   NetDev_Rx,                       /*   Rx                             */
                                                   NetDev_Tx,                       /*   Tx                             */
...
#define  RX_BUF_ALIGNMENT                                 16    /* Rx buffers MUST be aligned to a 16 byte boundary.    */
                           ...
                           NET_ERR  *perr)
{
    ...
    KAL_Dly(5);                                                         /* Requires a delay of a minimum of 64 internal bus cycles  */

    pdev->ETHERC_ECMR  &= ~(ECMR_RX_EN |                                /* Disable Rx and Tx                                        */
                            ECMR_TX_EN);
                                                                        /* ----- INITIALIZE EXTERNAL INTERRUPT CONTROLLER --------- */
    pdev->ETHERC_ECSIPR =   0x00000000L;                                /* Disable ALL dev interrupts.                              */
    ...
    if (*perr != NET_DEV_ERR_NONE) {
         return;
                                                                        ...
                                                                        /* --------------- VALIDATE DRIVER CFG -------------------- */
                                                                        /* See Note #8.                                             */
    if ((pdev_cfg->RxBufAlignOctets % RX_BUF_ALIGNMENT) != 0) {
       *perr = NET_DEV_ERR_INVALID_CFG;
        return;
    }

    if ((pdev_cfg->RxBufLargeSize % 16) != 0) {                         /* Rx buf size MUST be divisible by 16.                     */
       *perr = NET_DEV_ERR_INVALID_CFG;
        return;
    }
    ...
    if (pphy_cfg == (void *)0) {
       *perr = NET_IF_ERR_INVALID_CFG;                                  /* Call to NetIF_Start() does not specify Phy API.          */
        return;
    }

    if (pphy_cfg->BusMode != NET_PHY_BUS_MODE_MII) {                    /* Validate supported Phy bus types.                        */
        *perr = NET_DEV_ERR_INVALID_CFG;
         return;
    }
    ...
    pdev->EDMAC_FDR   = (FDR_RX_FIFO_512 |                              /* Configure the interrupt generation settings              */
                         FDR_TX_FIFO_512);
    ...
    Mem_PoolCreate ((MEM_POOL   *)&pdev_data->RxDescPool,               /* Pass a pointer to the mem pool to create.                */
                    (void       *) NET_DEV_SH_ETHERC_RAM_AREA,          /* From the onchip RAM.                                     */
                    (CPU_SIZE_T  ) NET_DEV_SH_ETHERC_RAM_SIZE,          /* Allocated RAM size.                                      */
                            ...
                            NET_ERR  *perr)
{
    ...
    pdev->EDMAC_EESIPR  = (EESIPR_RX      |                             /* Enable dev interrupts.                                   */
                           ...
                           EESIPR_RX_MULTI);
                                                                        /* ------------------ ENABLE RX & TX ---------------------- */
    pdev->EDMAC_EDRRR   =  DEF_ENABLED;                                 /* Enable dev device.                                       */
                           ...
                           NET_ERR  *perr)
{
    ...
    for (i = 0; i < pdev_cfg->TxDescNbr; i++) {                         /* See Note #2.                                             */
        ...
        do {
            ...
            switch (err) {
                ...
                default:
                     KAL_Dly(2);                                        /* Delay & retry if queue is full.                          */
                     loop = DEF_YES;
                     break;
            }
                                 ...
                                 NET_ERR  *perr)
{
    ...
    for (i = 0; i < free_cnt; i++) {
        ...
        if (*perr != NET_BUF_ERR_NONE) {
             plist->Next              = pdev_data->RxFreeListPtr;       /* Return list header to free list.                         */
             pdev_data->RxFreeListPtr = plist;
             break;
        }

        plist->Next                = pdev_data->RxBufferListPtr;        /* Store on buffer list.                                    */
        pdev_data->RxBufferListPtr = plist;
    }
                                    ...
                                    DEV_DESC  *pdesc)
{
    ...
    if ((pdesc->status & FE) != 0) {                                    /* Frame error, must advance curr ptr.                      */
        valid = DEF_FALSE;
    }

    if ((pdesc->status & (FP0 | FP1)) != (FP0 | FP1)) {                 /* Frame should be contained in a single buffer.            */
        valid = DEF_FALSE;
    }
