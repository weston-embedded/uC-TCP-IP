...
#define  BUF_ALIGN_OCTETS                           4u                                  /* Minimum buffer alignment.                            */
                                                                                        ...
                                                                                        /* ------------------ RX DESCRIPTORS ------------------ */
#define  GEM_RXBUF_ADDR_MASK                       (0xFFFFFFFCu)                        /* RX buffer address.                                   */
...
#define  GEM_RXBUF_SIZE_MASK                       (0x1FFFu)                            /* Size of frame.                                       */
                                                                                        ...
                                                                                        /* ------------------ TX DESCRIPTORS ------------------ */
#define  GEM_TXBUF_ADDR_MASK                       (0xFFFFFFFCu)                        /* TX Buffer address.                                   */
                            ...
                            NET_ERR  *p_err)
{
    ...
    pdev->NET_CFG     |= GEM_BIT_CFG_IGNORE_RX_FCS      |       /* Ignore Frame Check sequence.                         */
                         GEM_BIT_CFG_RX_CHKSUM_OFFLD_EN |       /* Checksum offloading.                                 */
                         GEM_BIT_CFG_FULL_DUPLEX        |       /* Full speed by default.                               */
                         GEM_BIT_CFG_SPEED;
    ...
    pdev->DMA_CFG |=  GEM_BIT_DMACFG_AHB_RX_SIZE(0x18u)  |      /* Max fifo depth.                                      */
    ...
    pdev->DMA_CFG &= ~GEM_BIT_DMACFG_AHB_PKT_SWAP_EN;
                         ...
                         NET_ERR      *p_err)
{
   ...
   *p_data = (CPU_INT08U *)(addr & GEM_RXBUF_ADDR_MASK);        /* Return a pointer to the newly received data area.    */
    ...
    if(pdesc == pdev_data->RxBufDescPtrEnd) {                   /* Update the descriptor to point to a new data area    */
        pdesc->Addr = ((CPU_INT32U)pbuf_new & GEM_RXBUF_ADDR_MASK) | GEM_RXBUF_ADDR_OWN | GEM_RXBUF_ADDR_WRAP;
    } else {
        pdesc->Addr = ((CPU_INT32U)pbuf_new & GEM_RXBUF_ADDR_MASK) | GEM_RXBUF_ADDR_OWN;
    }
                         ...
                         NET_ERR     *p_err)
{
    ...
    pdesc->Addr   = (CPU_INT32U)p_data & GEM_TXBUF_ADDR_MASK;
    ...
    pdev->TX_QBAR = (CPU_INT32U)pdev_data->TxRingHead;
                                 ...
                                 NET_ERR  *p_err)
{
    ...
    for (i = 0; i < pdev_cfg->RxDescNbr; i++) {
        ...
        pdesc->Addr = (CPU_INT32U)p_buf & ~GEM_RXBUF_ADDR_OWN;
    ...
    pdev->RX_QBAR = (CPU_INT32U)pdev_data->RxBufDescPtrStart;
                                   ...
                                   NET_ERR  *p_err)
{
    ...
    for (i = 0; i < pdev_cfg->RxDescNbr; i++) {                 /* Free Rx descriptor ring.                             */
        pdesc_data = (CPU_INT08U *)(pdesc->Addr & GEM_RXBUF_ADDR_MASK);
                                   ...
                                   NET_ERR  *p_err)
{
    ...
    while (pdev_data->TxRingSize > 0u) {
        NetIF_TxDeallocTaskPost((CPU_INT08U *)(pdesc->Status & GEM_TXBUF_LENGTH_MASK), &err);
        pdesc++;
   ...
   (void)type;                                                  /* Prevent 'variable unused' compiler warning.          */
    ...
    if (int_status & GEM_BIT_INT_TX_USED_READ) {
        ...
        while (pdev_data->TxRingSize < pdev_data->TxDescNbr) {
            ...
            pdesc->Addr   =  ((CPU_INT32U)tx_buf_q.DataPtr & GEM_TXBUF_ADDR_MASK);
        ...
        if (pdev_data->TxRingSize == 0u) {
            ...
            pdev->TX_QBAR = (CPU_INT32U)pdev_data->TxRingHead;
            CPU_MB();
            pdev->NET_CTRL |= GEM_BIT_CTRL_START_TX;            /* Start Tx if we had frames in the queue.              */
        }
    }
