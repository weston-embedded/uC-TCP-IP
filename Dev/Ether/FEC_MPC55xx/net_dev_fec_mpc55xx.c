...
*
*                                         FREESCALE FEC ON THE
*                                               MPC55xx
*
* Filename : net_dev_fec_mpc55xx.c
* Version  : V3.06.01
*********************************************************************************************************
* Note(s)  : (1) Assumes uC/TCP-IP V2.20.00 (or more recent version) is included in the project build.
*
*            (2) This driver accomodates MPC55xx errata 2049 where subsequent reads from the FEC
*                DMA hardware to descriptor memory yield invalid cached results.  This errata may
*                occur when :
*
*                (a) The receive DMA logic encounters a descriptor with the 'E' bit set (e.g.
*                    a receive over-run condition may have occured).  Clearing the 'E' bit in
*                    software SHOULD clear the event, however, the internal cache is NOT properly
*                    updated and thus the DMA logic is not made aware of the change in status.
*
*                (b) The transmit DMA logic encounters a descriptor with the 'R' bit clear (e.g.
*                    the descriptor being pointed to is NOT ready for transmission.  Clearing the
*                    'R' bit in software SHOULD clear the event, however, the internal cache is NOT
*                    properly updated and thus the DMA logic is not made aware of the change in
*                    status.
*
*            (3) Work-around(s) for errata 2049 include writting the FEC internal transmit and
*                receive descriptor pointer registers with the address of an alternate (temporary)
*                descriptor.  This technique can be implemented with minimal impact to the transmit
*                interrupt and buffer deallocation mechanism.
*
*            (4) For additional information, see MPC5553revAE.pdf, errata number 2049.
*
*            (5) If the FEC should become locked up due to errata 2049, then a mitigation attempt
*                is made to asynchronously correct the errata condition by periodically
*                checking for the errata and executing the solution.  The periodic scan occurs when
*                the Network Protocol Stack checks for link state.  Therefore, the longest amount of
*                time that the FEC is in failure due to errata is the period between checks.
*********************************************************************************************************
*/
...
#define    NET_DEV_FEC_MPC55XX_MODULE
#include  <Source/net.h>
#include  <Source/net_util.h>
#include  <IF/net_if.h>
#include  <IF/net_if_ether.h>
#include  "net_dev_fec_mpc55xx.h"
...
#define  FEC_RDAR_TO                                     100    /* Receive  complete polling timeout (See Note #2).     */
#define  FEC_TDAR_TO                                     100    /* Transmit complete polling timeout (See Note #2).     */
...
typedef  struct  net_dev  NET_DEV;                              /* Dev reg struct forward declaration.                  */
...
static  void  NetDev_RxErrataFix        (NET_DEV            *pdev);
...
*********************************************************************************************************
*/
                                                                                    /* FEC dev API fnct ptrs :          */
const  NET_DEV_API_ETHER  NetDev_API_FEC_MPC55XX = { NetDev_Init,                   /*   Init/add                       */
                                                     NetDev_Start,                  /*   Start                          */
                                                     NetDev_Stop,                   /*   Stop                           */
                                                     NetDev_Rx,                     /*   Rx                             */
                                                     NetDev_Tx,                     /*   Tx                             */
...
struct  net_dev {
    ...
    CPU_REG32  TFWR;
    CPU_REG32  RESERVED11;
    CPU_REG32  FRBR;
    CPU_REG32  FRSR;
    CPU_REG32  RESERVED12[11];
    CPU_REG32  ERDSR;
    CPU_REG32  ETDSR;
    CPU_REG32  EMRBR;
    CPU_REG32  XDES_CLOSEP_ADDR;
    CPU_REG32  RDES_CLOSEP_ADDR;
    CPU_REG32  RESERVED13[12];
    CPU_REG32  RDES_ADDR;
    CPU_REG32  XDES_ADDR;
};
...
#define  EIR_UN             (0x00080000)
#define  EIR_RL             (0x00100000)
#define  EIR_LC             (0x00200000)
#define  EIR_EBERR          (0x00400000)
#define  EIR_MII            (0x00800000)
#define  EIR_RXB            (0x01000000)
#define  EIR_RXF            (0x02000000)
#define  EIR_TXB            (0x04000000)
#define  EIR_TXF            (0x08000000)
#define  EIR_GRA            (0x10000000)
#define  EIR_BABT           (0x20000000)
#define  EIR_BABR           (0x40000000)
#define  EIR_HBERR          (0x80000000)

                                                                /* Bit definitions and macros for EIMR.                 */
#define  EIMR_UN            (0x00080000)
#define  EIMR_RL            (0x00100000)
#define  EIMR_LC            (0x00200000)
#define  EIMR_EBERR         (0x00400000)
#define  EIMR_MII           (0x00800000)
#define  EIMR_RXB           (0x01000000)
#define  EIMR_RXF           (0x02000000)
#define  EIMR_TXB           (0x04000000)
#define  EIMR_TXF           (0x08000000)
#define  EIMR_GRA           (0x10000000)
#define  EIMR_BABT          (0x20000000)
#define  EIMR_BABR          (0x40000000)
#define  EIMR_HBERR         (0x80000000)

                                                                /* Bit definitions and macros for RDAR.                 */
#define  RDAR_R_DES_ACTIVE  (0x01000000)

                                                                /* Bit definitions and macros for TDAR.                 */
#define  TDAR_X_DES_ACTIVE  (0x01000000)

                                                                /* Bit definitions and macros for ECR.                   */
#define  ECR_RESET          (0x00000001)
#define  ECR_ETHER_EN       (0x00000002)

                                                                /* Bit definitions and macros for MMFR.                 */
...
#define  MMFR_TA_10         (0x00020000)
                                                                ...
                                                                /* Bit definitions and macros for MSCR.                 */
#define  MSCR_MII_SPEED(x)  (((x) & 0x0000003F) <<  1)
#define  MSCR_DIS_PREAMBLE  (0x00000080)

                                                                /* Bit definitions and macros for MIBC.                 */
#define  MIBC_MIB_IDLE      (0x40000000)
#define  MIBC_MIB_DISABLE   (0x80000000)

                                                                /* Bit definitions and macros for RCR.                  */
#define  RCR_LOOP           (0x00000001)
#define  RCR_DRT            (0x00000002)
#define  RCR_MII_MODE       (0x00000004)
#define  RCR_PROM           (0x00000008)
#define  RCR_BC_REJ         (0x00000010)
#define  RCR_FCE            (0x00000020)
#define  RCR_MAX_FL(x)      (((x) & 0x000007FF) << 16)

                                                                /* Bit definitions and macros for TCR.                  */
#define  TCR_GTS            (0x00000001)
#define  TCR_HBC            (0x00000002)
#define  TCR_FDEN           (0x00000004)
#define  TCR_TFC_PAUSE      (0x00000008)
#define  TCR_RFC_PAUSE      (0x00000010)

#define  FRSR_VAL(x)        (((x) & 0x000000FF) <<  2)
                           ...
                           NET_ERR  *perr)
{
    ...
    if (pphy_cfg == (void *)0) {                                /* FEC must have ext Phy.                               */
    ...
    pdev->EIMR = 0x00000000;                                    /* Disable ALL FEC interrupts.                          */
    pdev->EIR  = 0xFFFFFFFF;                                    /* Clear all pending interrupts.                        */
    ...
    pdev->TCR = TCR_GTS;                                        /* Set the Graceful Transmit Stop bit.                  */
    while ((pdev->EIR & EIR_GRA) == 0) {
        KAL_Dly(1);                                             /* Wait for the current transmission to complete.       */
    }


    pdev->EIR  =  EIR_GRA;                                      /* Clear the GRA event.                                 */
    pdev->ECR  =  0;                                            /* Disable the FEC.                                     */
    pdev->TCR &= ~TCR_GTS;                                      /* Clear GTS bit so frames can be tx'd when restarted.  */


    pdev->ECR = ECR_RESET;                                      /* Perform soft reset of FEC.                           */
    while ((pdev->ECR & ECR_RESET) != 0) {
        KAL_Dly(1);
    }


    pdev->RCR  = RCR_MAX_FL(pdev_cfg->RxBufLargeSize);          /* Set maximum Rx frame len.                            */


    pdev->TCR |= TCR_FDEN;                                      /* !!!! Set full duplex despite link duplex unknown.    */


                                                                /* Retrieve FEC module clk freq.                        */
    clk = pdev_bsp->ClkFreqGet(pif, perr);
    if (*perr != NET_DEV_ERR_NONE) {
         return;
    }

    clk /= 2500000;                                             /* prescaler for 2.5MHz.                                */
    clk /= 2;                                                   /* MII_SPEED = prescaler / 2.                           */
    if (clk == 0) {
        clk = 0x0F;                                             /* Set prescaler to 0xF in case calculation fails.      */
    }

    pdev->MSCR = MSCR_MII_SPEED(clk);                           /* Set MII bus freq less than or equal to 2.5Mhz.       */


                                                                /* Initialize MIB counters to 0.                        */
    mib_base = (CPU_INT32U *)(pdev_cfg->BaseAddr + MIB_REG_CTR_OFFSET_START);
    while (mib_base <= (CPU_INT32U *)(pdev_cfg->BaseAddr + MIB_REG_CTR_OFFSET_END)) {
       *mib_base++ = 0;
    }

    pdev->MIBC = 0;                                             /* Enable MIB counters.                                 */


    pdev->FRSR = FRSR_VAL(0x48);                                /* Set minimum Rx Start FIFO value.                     */


                                                                /* -------------- CFG PHY OPERATING MODE -------------- */
    if (pphy_cfg->BusMode == NET_PHY_BUS_MODE_MII) {
        pdev->RCR |=  RCR_MII_MODE;                             /* Set MII bus mode to MII.                             */
    }

    if (pphy_cfg->BusMode == NET_PHY_BUS_MODE_RMII) {
        pdev->RCR &= ~RCR_MII_MODE;                             /* Set MII bus mode to RMII.                            */
    }
                            ...
                            NET_ERR  *perr)
{
    ...
    if (err == NET_IF_ERR_NONE) {                               /* If no errors, configure device    HW MAC address.    */
        ...
        if (hw_addr_cfg != DEF_YES) {                           /* Else attempt to get device's automatically loaded ...*/
            ...
            hw_addr[0] = (pdev->PALR >> (3 * DEF_INT_08_NBR_BITS)) & DEF_INT_08_MASK;
            hw_addr[1] = (pdev->PALR >> (2 * DEF_INT_08_NBR_BITS)) & DEF_INT_08_MASK;
            hw_addr[2] = (pdev->PALR >> (1 * DEF_INT_08_NBR_BITS)) & DEF_INT_08_MASK;
            hw_addr[3] = (pdev->PALR >> (0 * DEF_INT_08_NBR_BITS)) & DEF_INT_08_MASK;

            hw_addr[4] = (pdev->PAUR >> (3 * DEF_INT_08_NBR_BITS)) & DEF_INT_08_MASK;
            hw_addr[5] = (pdev->PAUR >> (2 * DEF_INT_08_NBR_BITS)) & DEF_INT_08_MASK;
    ...
    if (hw_addr_cfg == DEF_YES) {                               /* If necessary, set device HW MAC address.             */
        pdev->PALR = (((CPU_INT32U)hw_addr[0] << (3 * DEF_INT_08_NBR_BITS)) |
        ...
        pdev->PAUR = (((CPU_INT32U)hw_addr[4] << (3 * DEF_INT_08_NBR_BITS)) |
    ...
    pdev->IAUR = 0;                                             /* Individual address upper register.                   */
    pdev->IALR = 0;                                             /* Individual address lower register.                   */
    pdev->GAUR = 0;                                             /* Group (multicast) address upper register.            */
    pdev->GALR = 0;                                             /* Group (multicast) address lower register.            */
    ...
    if (*perr != NET_DEV_ERR_NONE) {
         return;
                                                                ...
                                                                /* -------------------- CFG INT'S --------------------- */
    pdev->EIR  = (EIR_RXF  | EIR_RXB);                          /* Clear the Rx interrupt flags.                        */
    pdev->EIR  = (EIR_TXF  | EIR_TXB);                          /* Clear the Tx interrupt flags.                        */
    pdev->EIMR = (EIMR_RXF | EIMR_TXF);                         /* Enable FEC interrupts.                               */


                                                                /* ------------------ ENABLE RX & TX ------------------ */
    pdev->ECR  = ECR_ETHER_EN;                                  /* Enable the FEC.                                      */


                                                                /* ----------------- START DMA FETCH ------------------ */
    pdev->RDAR = RDAR_R_DES_ACTIVE;                             /* Tell HW that there is an empty Rx buffer produced.   */
                           ...
                           NET_ERR  *perr)
{
    ...
    pdev      = (NET_DEV           *)pdev_cfg->BaseAddr;        /* Overlay dev reg struct on top of dev base addr.      */
                                                                ...
                                                                /* ----------------- DISABLE RX & TX ------------------ */
    pdev->ECR  &=  ~ECR_ETHER_EN;                               /* Disable the FEC.                                     */


                                                                /* -------------- DISABLE & CLEAR INT'S --------------- */
    pdev->EIMR &= ~(EIMR_RXF | EIMR_TXF);                       /* Disable FEC interrupts.                              */
    pdev->EIR  &= ~(EIR_RXF  | EIR_RXB);                        /* Clear the Rx interrupt flags.                        */
    pdev->EIR  &= ~(EIR_TXF  | EIR_TXB);                        /* Clear the Tx interrupt flags.                        */
                         ...
                         NET_ERR      *perr)
{
   ...
   *p_data = (CPU_INT08U *)pdesc->p_data;                       /* Return a pointer to the newly received data area.    */
    ...
    pdesc->p_data = pbuf_new;                                   /* Update the descriptor to point to a new data area.   */

    NetDev_RxDescPtrCurInc(pif);                                /* Free the current descriptor.                         */

                                                                /*  ------------- FEC ERRATA WORKAROUND --------------- */
    if ((pdev->RDAR & RDAR_R_DES_ACTIVE) == 0) {
        NetDev_RxErrataFix(pdev);                               /* See Notes #2, #3, & #4 net_dev_fec_mpc55xx.c.        */
    }
                         ...
                         NET_ERR     *perr)
{
    ...
    NET_DEV            *pdev;
    DEV_DESC           *pdesc;
    DEV_DESC            tx_desc_dummy;
    CPU_REG32           status;
    CPU_INT32U          xdes_addr;
    CPU_INT32U          xdes_closep_addr;
    CPU_INT32U          timeout;
    ...
    pdesc     = (DEV_DESC          *)pdev_data->TxBufDescPtrCur;/* Obtain ptr to next available Tx descriptor.          */
                                                                ...
                                                                /*  ------------- FEC ERRATA WORKAROUND --------------- */
                                                                /* See Notes #2, #3, & #4 net_dev_fec_mpc55xx.c.        */
    if ((pdev->TDAR & TDAR_X_DES_ACTIVE) == 0)                  /* Transmitter idle?                                    */
        pdesc       = (DEV_DESC *)pdev->XDES_ADDR;              /* Obtain addr of desc pointed to by FEC HW.            */
        desc_status =  pdesc->status & TX_BD_R;
        if (desc_status == 0) {                                 /* Transmitter idle due to desc with 0 'R' bit?         */
            tx_desc_dummy.status   = (CPU_INT16U) 0;            /* Prepare alternate desc with 'R' bit clr.             */
                                                                /* Save FEC internal desc ptr.                          */
            xdes_addr              = (CPU_INT32U) pdev->XDES_ADDR;
            xdes_closep_addr       = (CPU_INT32U) pdev->XDES_CLOSEP_ADDR;
                                                                /* Cfg FEC internal ptr with alternate desc addr.       */
            pdev->XDES_ADDR        = (CPU_INT32U)&tx_desc_dummy;
            pdev->XDES_CLOSEP_ADDR = (CPU_INT32U)&tx_desc_dummy;

            pdev->TDAR = TDAR_X_DES_ACTIVE;                     /* Load alternate desc.  Expect Tx failure.             */

            timeout = FEC_TDAR_TO;                              /* Wait for Tx failure due to desc status 'R' bit clr.  */
            status  = pdev->TDAR & TDAR_X_DES_ACTIVE;
            while ((timeout > 0) &&
                   (status  > 0)) {
                timeout--;
                status = pdev->TDAR & TDAR_X_DES_ACTIVE;
            ...
            pdev->XDES_ADDR        = (CPU_INT32U)xdes_addr;     /* Restore FEC internal desc ptr regs to orig val's.    */
            pdev->XDES_CLOSEP_ADDR = (CPU_INT32U)xdes_closep_addr;
        }
    ...
    if (desc_status == 0) {                                     /* Find next available Tx descriptor (see Note #3).     */
        ...
        pdev->TDAR = TDAR_X_DES_ACTIVE;                         /* Load desc & Tx.                                      */
                              ...
                              NET_ERR     *perr)
{
    ...
    switch (opt) {
        case NET_IF_IO_CTRL_LINK_STATE_GET_INFO:
                                                                /*  ------------- FEC ERRATA MITIGATION --------------- */
             if ((pdev->RDAR & RDAR_R_DES_ACTIVE) == 0) {
                 NetDev_RxErrataFix(pdev);                      /* See Notes #2, #3, #4, &5 net_dev_fec_mpc55xx.c.      */
             }

             plink_state = (NET_DEV_LINK_ETHER *)p_data;        /* Obtain link state.                                   */
             ...
             if (plink_state->Duplex != duplex) {
                 switch (plink_state->Duplex) {
                    case NET_PHY_DUPLEX_FULL:
                         pdev->TCR |=  TCR_FDEN;                /* !!!! ECR[ETHER_EN] must equal 0 before changing TCR. */
                         break;
                             ...
                             NET_ERR     *perr)
{
    ...
    pdev->EIR  = EIR_MII;                                       /* Clear the MII interrupt bit.                         */

    pdev->MMFR = MMFR_ST_01        |                            /* Execute a Phy register read command.                 */
                 MMFR_OP_READ      |
                 MMFR_PA(phy_addr) |
                 MMFR_RA(reg_addr) |
                 MMFR_TA_10;
    ...
    pdev->EIR = EIR_MII;                                        /* Clear the interrupt bit.                             */

   *p_data  = pdev->MMFR & 0xFFFF;
                             ...
                             NET_ERR     *perr)
{
    ...
    pdev->MMFR = MMFR_ST_01        |                            /* Write to the Phy register.                           */
                 MMFR_OP_WRITE     |
                 MMFR_PA(phy_addr) |
                 MMFR_RA(reg_addr) |
                 MMFR_TA_10        |
                                 ...
                                 NET_ERR  *perr)
{
    ...
    pdev->EMRBR =  pdev_cfg->RxBufLargeSize;                    /* Set maximum receive buffer size.                     */
    pdev->ERDSR = (CPU_INT32U)pdev_data->RxBufDescPtrStart;     /* Cfg DMA descriptor ring start address.               */
...
static  void  NetDev_RxDescPtrCurInc (NET_IF *pif)
{
    ...
    pdesc->status = wrap  | RX_BD_E;                            /* Free the descriptor.                                 */
                                 ...
                                 NET_ERR *perr)
{
    ...
    for (i = 0; i < pdev_cfg->TxDescNbr; i++) {                 /* Initialize Tx descriptor ring.                       */
        ...
        pdesc++;                                                /* Point to next descriptor in list.                    */
                                                                ...
                                                                /* ------------- INIT HARDWARE REGISTERS -------------- */
    pdev->ETDSR = (CPU_INT32U)pdev_data->TxBufDescPtrStart;     /* Configure the DMA with the Tx desc start address.    */
                                  ...
                                  NET_DEV_ISR_TYPE   type)
{
   ...
   (void)&type;                                                 /* Prevent compiler warning if arg 'type' not req'd.    */
    ...
    reg_val   = pdev->EIR;
    pdev->EIR = reg_val;                                        /* Clear the interrupt.                                 */

                                                                /* ------------------ HANDLE RX ISRs ------------------ */
    if ((reg_val & (EIR_RXF | EIR_RXB)) > 0) {
        n_rdy = 0;
        pdesc = pdev_data->RxBufDescPtrStart;
                ...
                case NET_IF_ERR_NONE:
                 ...
                 default:
                      ...
                      break;
                                                                ...
                                                                /* ------------------ HANDLE TX ISRs ------------------ */
    if ((reg_val & (EIR_TXF | EIR_TXB)) > 0) {
        do {                                                    /* Post ALL Tx'd buffers to dealloc task (see Note #4). */
            NetIF_DevTxRdySignal(pif);                          /* Signal Net IF that Tx resources are available.       */
            pdesc = pdev_data->TxBufDescCompPtr;
            if (((pdesc->status  & TX_BD_R)  == 0) &&
                ((pdesc->status  & TX_BD_TO1) > 0)) {
                pdesc->status  &= ~TX_BD_TO1;
                NetIF_TxDeallocTaskPost(pdesc->p_data, &err);
        ...
        } while (((pdesc->status & TX_BD_R) == 0) && ((pdesc->status & TX_BD_TO1) > 0));
    }
}
...
*                                            NetDev_RxErrataFix()
*
* Description : This function removes the FEC from lockup when experiencing Rx errata 2049.
*
* Argument(s) : pdev    Pointer to interface device registers.
*               ---
*
* Return(s)   : none.
*
...
*               NetDev_IO_Ctrl().
*
* Note(s)     : (1) See Notes #2, #3, #4, & #5 net_dev_fec_mpc55xx.c.
*********************************************************************************************************
*/
...
{
    CPU_INT32U  status;
    CPU_INT32U  rdes_addr;
    CPU_INT32U  rdes_closep_addr;
    CPU_INT32U  timeout;
    DEV_DESC    rx_desc_dummy;
    CPU_SR_ALLOC();
                                                                /*  ------------- FEC ERRATA WORKAROUND --------------- */
                                                                /* See Notes #2, #3, #4, & #5 net_dev_fec_mpc55xx.c.    */
    CPU_CRITICAL_ENTER();

    rx_desc_dummy.status   = (CPU_INT16U) 0;                    /* Prepare alternate desc with 'E' bit clr.             */
    rdes_addr              = (CPU_INT32U) pdev->RDES_ADDR;      /* Save FEC internal desc ptr.                          */
    rdes_closep_addr       = (CPU_INT32U) pdev->RDES_CLOSEP_ADDR;
    pdev->RDES_ADDR        = (CPU_INT32U)&rx_desc_dummy;        /* Cfg FEC internal ptr with alternate desc addr.       */
    pdev->RDES_CLOSEP_ADDR = (CPU_INT32U)&rx_desc_dummy;

    pdev->RDAR = RDAR_R_DES_ACTIVE;                             /* Load alternate desc.  Expect Rx failure.             */

    timeout = FEC_RDAR_TO;                                      /* Wait for Rx failure due to desc status 'E' bit clr.  */
    status  = pdev->RDAR & RDAR_R_DES_ACTIVE;
    while ((timeout > 0) &&
           (status  > 0)) {
        timeout--;
        status = pdev->RDAR & RDAR_R_DES_ACTIVE;
    ...
    pdev->RDES_ADDR        = (CPU_INT32U)rdes_addr;             /* Restore FEC internal desc ptr regs to orig val's.    */
    pdev->RDES_CLOSEP_ADDR = (CPU_INT32U)rdes_closep_addr;

    pdev->RDAR = RDAR_R_DES_ACTIVE;                             /* Load orig desc.  Resume normal operation.            */
