...
*                                            GMAC 10/100
*
* Filename : net_dev_rz_ether_cfg.c
* Version  : V3.06.01
*********************************************************************************************************
...
#define    NET_DEV_RZ_ETHER_CFG_MODULE
...
const  NET_DEV_CFG_ETHER  NetDev_Cfg_RTK772100FC = {
    ...
    1536u,                          /* Desired size      of device's large receive  buffers (in octets) [see Note #2].  */
      76u,                          /* Desired number    of device's large receive  buffers             [see Note #3a]. */
      64u,                          /* Desired alignment of device's       receive  buffers (in octets) [see Note #4].  */
    ...
    1618u,                          /* Desired size      of device's large transmit buffers (in octets) [see Note #2].  */
      16u,                          /* Desired number    of device's large transmit buffers             [see Note #3b]. */
     156u,                          /* Desired size      of device's small transmit buffers (in octets) [see Note #2].  */
      16u,                          /* Desired number    of device's small transmit buffers             [see Note #3b]. */
      64u,                          /* Desired alignment of device's       transmit buffers (in octets) [see Note #4].  */
    ...
    NET_DEV_CFG_FLAG_NONE,          /* Desired option flags, if any (see Note #6).                                      */
      ...
      38u,                          /* Desired number of device's receive  descriptors (see Note #7).                   */
      32u,                          /* Desired number of device's transmit descriptors.                                 */

    0xE8203000u,                    /* Base address   of device's hardware/registers.                                   */

            32u,                    /* Size           of device's data bus (in bits), if available.                     */


   "00:AB:CD:EF:80:FF",             /* Desired device hardware address; may be NULL address or string ...               */
                                    /* ... if  device hardware address configured or set at run-time.                   */
};


const  NET_PHY_CFG_ETHER  NetPhy_Cfg_RTK772100FC = {
