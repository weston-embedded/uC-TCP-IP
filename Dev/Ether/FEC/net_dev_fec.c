...
#define    NET_DEV_FEC_MODULE
#include  <Source/net.h>
#include  <Source/net_util.h>
#include  <IF/net_if.h>
#include  <IF/net_if_ether.h>
#include  <KAL/kal.h>
#include  "net_dev_fec.h"
...
const  NET_DEV_API_ETHER  NetDev_API_FEC = { NetDev_Init,                           /*   Init/add                       */
                                             NetDev_Start,                          /*   Start                          */
                                             NetDev_Stop,                           /*   Stop                           */
                                             NetDev_Rx,                             /*   Rx                             */
                                             NetDev_Tx,                             /*   Tx                             */
                         ...
                         NET_ERR     *perr)
{
    ...
    if (desc_status == 0) {                                     /* Find next available Tx descriptor (see Note #3).     */
        ...
        pdesc->status = (wrap     |                             /* Hardware owns the descriptor, enable Tx Comp. Int's  */
                         TX_BD_R  |
                         TX_BD_L  |
                         TX_BD_TC |
                         TX_BD_TO1);                            /* See note #4.                                         */

        pdev->TDAR = TDAR_X_DES_ACTIVE;                         /* Instruct DMA to transmit descriptor data.            */
                                  ...
                                  NET_DEV_ISR_TYPE   type)
{
   ...
   (void)&type;                                                 /* Prevent compiler warning if arg 'type' not req'd.    */
    ...
    if ((reg_val & (EIR_RXF | EIR_RXB)) > 0) {
        ...
        pdev->EIR = EIR_RXF | EIR_RXB;                          /* Clear the interrupt.                                 */
    }
    ...
    if ((reg_val & (EIR_TXF | EIR_TXB)) > 0) {
        ...
        pdev->EIR = EIR_TXF | EIR_TXB;                          /* Clear the interrupt.                                 */
    }
}
